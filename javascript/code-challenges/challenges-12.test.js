...
const validateWord = (word) => {
  return /^[a-zA-Z0]{5,10}$/gm.test(word);
};
...
const hasNumber = (string) => {
  return /[a-zA-Z][0-9]+/i.test(string);
};
...
const validateEmail = (email) => {
  return -1 !== (email + '').search(/^[a-zA-Z0-9]+\.?[a-zA-Z0-9]+@[a-zA-Z0-9]+(\.net|\.com|\.org)$/);
};
...
const validatePhoneNumber = (phoneNumber) => {
  return -1 !== (phoneNumber + '').search(/^(\([0-9]{3}\)|[0-9]{3})\s?-?[0-9]{3}\s?-?[0-9]{4}$/);
};
