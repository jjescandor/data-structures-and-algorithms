...
const addValues = (arr) => {
  return arr.length ? arr.reduce((accumulator, currentValue) => accumulator + currentValue) : 0;
};
...
const addPurchases = (arr) => {
  return arr.length ? arr.map(value => value.purchasePrice).reduce((acc, curr) => acc + curr) : 0;
};
...
const returnNames = (arr) => {
  return arr.reduce((acc, curr) => typeof acc === 'string' ? acc + `,${curr.name}` : acc.name + `,${curr.name}`).split(',');
};
...
const extractChildren = (arr) => {
  return arr.filter(value => value.name.includes('a') && ('children' in value)).map(value => value.children).reduce((acc, curr) => acc.concat(curr));
};
